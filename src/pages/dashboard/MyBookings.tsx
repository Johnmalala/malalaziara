...
export const MyBookings: React.FC = () => {
  ...
  const getStatusBadge = (booking: BookingWithListing) => {
    ...
    return <span className={`px-3 py-1 text-xs font-semibold rounded-full capitalize ${statusMap[booking.status] || 'bg-gray-100 text-gray-800'}`}>{booking.status}</span>;
  };
            ...
            <div key={booking.id} className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col md:flex-row items-start md:space-x-6">
              ...
              <div className="flex-1 w-full mt-4 md:mt-0">
                ...
                <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700 flex items-center space-x-3">
                  ...
                  {booking.status === 'pending' && <Button variant="ghost" size="sm" className="text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/50" onClick={() => handleCancelBooking(booking.id)}>Cancel</Button>}
                </div>
              </div>
            </div>
          ))}
